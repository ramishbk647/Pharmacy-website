...
<body>
	...
	<?php
		...
		function UpdateDatabaseInformation(string $saleID, string $customer, string $saleTime, string $taxAmount, string $discountAmount, string $product, string $amount, string $price) {
			...
			if (empty($saleTime)) {
				$queryReturns = QueryDatabase(
					$connection,
					'UPDATE `SalesRecords` SET `customerID` = ?, `saleTime` = CURTIME(), `taxAmount` = ?, `discountAmount` = ? WHERE `id` = ?',
					array($customerID, $taxAmount, $discountAmount, $saleID)
				);
				if (!$queryReturns[0]) {
					$connection->close();
					return $returnHeader.$queryReturns[1];
				...
				$queryReturns = QueryDatabase(
					$connection,
					'UPDATE `SalesRecords` SET `customerID` = ?, `saleTime` = ?, `taxAmount` = ?, `discountAmount` = ? WHERE `id` = ?',
					array($customerID, $saleTime, $taxAmount, $discountAmount, $saleID)
				);
				if (!$queryReturns[0]) {
					$connection->close();
					return $returnHeader.$queryReturns[1];
			...
			$queryObj = $connection->prepare('UPDATE `SalesProducts` SET `productID` = ?, `amount` = ?, `price` = ? WHERE `saleID` = ?');
			$queryObj->bind_param('isss', $productID, $amount, $price, $saleID);
		...
		{
			$saleID = GetFormStringValue('sale_id');
			...
			$errMsg = UpdateDatabaseInformation($saleID, $customer, $saleTime, $taxAmount, $discountAmount, $product, $amount, $price);
		}
	?>
	<div class="site-wrap">
			...
			<div class="container">
				<?php
					if ($errMsg)
					{
						echo('<div class="row">
							<div class="col-md-12">
								<h2 class="h3 mb-5 text-black">Sales record modification failed: '.$errMsg.'</h2>
							</div>
							<div class="row">
								<div class="col-lg-12">
									<a href="sales_record_edit.php?id='.$saleID.'">
					...
					else
					{
						echo('<div class="row row_continue">
							<div class="col-md-12">
								<h2 class="h3 mb-5 text-black">Sales Record Modified Successfully!</h2>
							</div>
							<div class="row">
								<div class="col-lg-12">
